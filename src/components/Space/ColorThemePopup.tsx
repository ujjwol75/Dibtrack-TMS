...
const ColorThemePopup = ({handleSubmit , isopen , closeModal ,openModal}: Props) => {
  ...
  return (
    <div>
     <button  className='w-full bg-btncolor mt-5 p-4 text-white ' onClick={openModal}>Next</button>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-[800px] h-[500px] transform overflow-hidden rounded-2xl bg-white p-6 text-left shadow-xl transition-all">
                 ...
                 <div className=" w-full mt-2 bg-bgsearchbar">
                       <div className="flex flex-row justify-around p-4 border-b border-gray-200">
                           <ChevronLeftIcon className="h-6 w-6 text-gray-400 mt-3" onClick={closeModal}/>
                           <span className="text-gray-500 text-2xl">Space color or Avatar</span>
                           <span className="py-2 px-3 bg-blue-400 " onClick={closeModal}>
                               <span className="hover:rotate-180 flex justify-center items-center">X</span></span>
                       </div>

                       <div className="w-full h-[500px] bg-bgsearchbar flex flex-row  mt-20 pl-20">
                                    <CircleUserIcon1 size="lg" color = {color} />
                                    <div className="ml-12">
                                        <span className="text-gray-400 text-md">SPACE COLORS</span>
                                          <div className="flex flex-row mt-2">
                                              {ThemeColors.map((ThemeColor) =>(
                                                 <ColorDiv color={ThemeColor} setThemeColor = {setThemeColor} />  
                                              ) )}
